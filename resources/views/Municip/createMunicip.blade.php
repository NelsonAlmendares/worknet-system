...
<div class="d-flex">
    ...
    <div class="main">
        ...
        <div class="p-4">
            <h2 class="greet">Manejo de Municipios</h2>

            <div class="mt-5">
                <div class="">
                    <h3 class="text-center mb-3">Listado de Municipios registrados</h3>
                    @if($municips->isEmpty())
                        <p>Aún no hay municipios registrados.</p>
                    @else
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($municips as $municip)
                            <tr class="text-center">
                                <td>{{ $municip->idmunicip }}</td>
                                <td>{{ $municip->municipname }}</td>
                                <td>{{ $municip->municip_idmh }}</td>
                                <td>{{ $municip->department->deptname }}</td>
                                <td>{{ $municip->estado_descripcion }}</td>
                                <td>

                                    <a href="{{ route('Municip.edit', $municip->idmunicip) }}" class="btn btn-warning btn-sm">
                                        Editar <i class='bx bxs-edit-alt'></i>
                                    </a>

                                    <form id="deleteForm-{{ $municip->idmunicip }}" action="{{ route('Municip.destroy', $municip->idmunicip) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm fw-bold" onclick="confirmDelete({{ $municip->idmunicip }})">
            ...
            <div class="add-center">
                <button type="button" class="btn btn-secondary btn-custom-size fw-bold" data-bs-toggle="modal" data-bs-target="#exampleModal">
                    Agregar municipio <i class='bx bx-user-pin custom-icon-size' ></i>
                </button>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Municipio</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="card-body">

                                <form action="{{ route('Municip.store') }}" method="POST">
                                    @csrf
                                    <!-- Nombre y Apellido -->
                                    <div class="mb-3">
                                        <label for="municipname" class="form-label">Nombre</label>
                                        <input type="text" class="form-control" name="municipname" id="municipname" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="municip_idmh" class="form-label">IDMH</label>
                                        <input type="text" class="form-control" name="municip_idmh" id="municip_idmh" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="municip_iddepartment" class="form-label">Departamento</label>
                                        <select class="form-select" id="municip_iddepartment" name="municip_iddepartment" required>
                                            <option value="">Seleccione un departamento</option>
                                            @foreach ($departments as $department)
                                                <option value="{{ $department->iddepartment }}" {{ old('municip_iddepartment') == $department->iddepartment ? 'selected' : '' }}>
                                                    {{ $department->deptname }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        @endphp
                                    <div class="mb-3">
                                        <label for="municip_e" class="form-label">Status</label>
                                        <select class="form-select" id="municip_e" name="municip_e" required>
                                            <option value="">Elija el Status</option>
                                            @foreach ($estados as $key => $label)
                                                <option value="{{ $key }}" {{ old('municip_e') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <button type="submit" class="btn btn-success fw-bold">
                                        Enviar municipio <i class='bx bx-user-plus custom-icon-size' ></i>
                                    </button>
                                </form>
                            </div>
                        </div>
...
<script>
    ...
    function confirmDelete(municipId) {
        Swal.fire({
            title: '¿Estás seguro?',
            ...
            if (result.isConfirmed) {
                document.getElementById(`deleteForm-${municipId}`).submit();
                setTimeout(() => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Éxito',
