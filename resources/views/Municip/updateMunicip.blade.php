...
<div class="d-flex">
    <div class="sidebar">
        ...
        <div>
            <div class="menu-item" data-bs-toggle="collapse" data-bs-target="#distritos">
                <span class="font_custom-white">
            ...
            <div class="collapse submenu" id="distritos">
        ...
        <div>
            <div class="menu-item" id="active"  data-bs-toggle="collapse" data-bs-target="#municips">
                <span class="font_custom-white">
          ...
          <div class="p-4">
            ...
            <div class="mt-5">
                <div class="card container p-3">
                    <h3 class="text-center mb-3">Actualizar Municipio</h3>

                    <form action="{{ route('Municip.update', $municip->idmunicip) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="municipname" id="municipname" value="{{ $municip->municipname }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="municip_idmh" class="form-label">IDMH</label>
                            <input type="text" class="form-control" name="municip_idmh" id="municip_idmh" value="{{ $municip->municip_idmh }}" required>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="municip_iddepartment" name="municip_iddepartment" required>
                                ...
                                @foreach ($departments as $department)
                                    <option value="{{ $department->iddepartment }}" {{ old('municip_iddepartment', $municip->department->iddepartment ?? '') == $department->iddepartment ? 'selected' : '' }}>
                                        {{ $department->deptname }}
                                    </option>
                                @endforeach
                            </select>
...
@endphp
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="municip_e" name="municip_e" required>
                                ...
                                @foreach ($estados as $key => $label)
                                    <option value="{{ $key }}" {{ old('municip_e', $municip->municip_e ?? '') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="update-option">
                            <button type="submit" class="btn btn-primary espaciado">Actualizar Municipio</button>
                            <a href="{{ route('Municip.index') }}" class="btn btn-secondary espaciado">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        @if (Session::has('success'))
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "{{ url('municip') }}";
            }
        });
        @endif
