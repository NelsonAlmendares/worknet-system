          ...
          <div class="p-4">
            ...
            <div class="mt-5">
                <div class="card p-3">
                    ...
                    @if($municipsnew->isEmpty())
                        <p>Aún no hay municipios (nuevo) registrados.</p>
                    @else
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($municipsnew as $municip)
                            <tr class="text-center">
                                <td>{{ $municip->idmunicipnew }}</td>
                                <td>{{ $municip->municipnewname }}</td>
                                <td>{{ $municip->municipnew_idmh }}</td>
                                <td>{{ $municip->department->deptname }}</td>
                                <td>{{ $municip->estado_descripcion }}</td>
                                <td>

                                    <a href="{{ route('MunicipNew.edit', $municip->idmunicipnew) }}" class="btn btn-warning btn-sm">Editar</a>

                                    <form id="deleteForm-{{ $municip->idmunicipnew }}" action="{{ route('MunicipNew.destroy', $municip->idmunicipnew) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm fw-bold" onclick="confirmDelete({{ $municip->idmunicipnew }})">
            ...
            <div class="add-center">
                <button type="button" class="btn btn-secondary btn-custom-size fw-bold" data-bs-toggle="modal" data-bs-target="#exampleModal">
                    Agregar municipio (nuevo) <i class='bx bx-user-pin custom-icon-size' ></i>
                </button>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Municipio (nuevo)</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="card-body">

                                <form action="{{ route('MunicipNew.store') }}" method="POST">
                                    @csrf
                                    <!-- Nombre y Apellido -->
                                    <div class="mb-3">
                                        <label for="municipnewname" class="form-label">Nombre</label>
                                        <input type="text" class="form-control" name="municipnewname" id="municipnewname" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="municipnew_idmh" class="form-label">IDMH</label>
                                        <input type="text" class="form-control" name="municipnew_idmh" id="municipnew_idmh" required>
                                    </div>
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="municipnew_iddept" name="municipnew_iddept" required>
                                            ...
                                            @foreach ($departments as $department)
                                                <option value="{{ $department->iddepartment }}" {{ old('municipnew_iddept') == $department->iddepartment ? 'selected' : '' }}>
                                                    {{ $department->deptname }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="municipnew_e" name="municipnew_e" required>
                                            ...
                                            @foreach ($estados as $key => $label)
                                                <option value="{{ $key }}" {{ old('municipnew_e') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <button type="submit" class="btn btn-success fw-bold">
                                        Crear municipio (nuevo)<i class='bx bx-user-plus custom-icon-size' ></i>
                                    </button>
                                </form>
                            </div>
                        </div>
