...
<div class="d-flex">
    <div class="sidebar">
        ...
        <div>
            <div class="menu-item" id="active" data-bs-toggle="collapse" data-bs-target="#department">
                <span class="font_custom-white">
    ...
    <div class="main">
        <nav class="navbar navbar-expand-lg bg-body-tertiary">
          ...
          <div class="p-4">
            <h2 class="greet">Manejo de Departmentos</h2>

            <div class="mt-5">
                <div class="">
                    <h3 class="text-center mb-3">Listado de Departmentos registrados</h3>
                    @if($departments->isEmpty())
                        <p>Aún no hay departamentos registrados.</p>
                    @else
                    ...
                    <tbody>
                        @foreach ($departments as $department)
                            <tr class="text-center">
                                ...
                                <td>{{ $department->dept_idmh }}</td>
                                <td>{{ $department->estado_descripcion }}</td>
                                <td>

                                    <a href="{{ route('Department.edit', $department->iddepartment) }}" class="btn btn-warning btn-sm">
                                        Editar <i class='bx bxs-edit-alt' ></i>
                                    </a>

                                    <form id="deleteForm-{{ $department->iddepartment }}" action="{{ route('Department.destroy', $department->iddepartment) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm fw-bold" onclick="confirmDelete({{ $department->iddepartment }})">
                        ...
                        <div class="modal-body">
                            <div class="card-body">
                                ...
                                <form action="{{ route('Department.store') }}" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <label for="dept_idmh" class="form-label">IDMH</label>
                                        <input type="text" class="form-control" name="dept_idmh" id="dept_idmh" required>
                                    </div>
                                        @php
                                            $estados = [
                                                    'A' => 'Activo',
                                        ...
                                        @endphp
                                    <div class="mb-3">
                                        <label for="dept_e" class="form-label">Status</label>
                                        <select class="form-select" id="dept_e" name="dept_e" required>
                                            <option value="">Elija el Status</option>
                                            @foreach ($estados as $key => $label)
                                                <option value="{{ $key }}" {{ old('dept_e') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
