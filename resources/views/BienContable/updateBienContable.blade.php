            ...
            <div class="mt-5">
                <div class="card container p-3">
                    ...
                    <form action="{{ route('BienContable.update', $bien->id_tb_contable) }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="tbc_codigo_contable" class="form-label">Nombre / Código</label>
                            <input type="text" class="form-control" name="tbc_codigo_contable" id="tbc_codigo_contable" value="{{ $bien->tbc_codigo_contable }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="tbc_desc" class="form-label">Descripción</label>
                            <input type="text" class="form-control" name="tbc_desc" id="tbc_desc" value="{{ $bien->tbc_desc }}" required>
                        </div>
@php
$estados = [
'A' => 'Activo',
...
@endphp
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="tbc_e" name="tbc_e" required>
                                ...
                                @foreach ($estados as $key => $label)
                                    <option value="{{ $key }}" {{ old('tbc_e', $bien->tbc_e ?? '') == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="update-option">
                            ...
                            <a href="{{ route('BienContable.index') }}" class="btn btn-secondary espaciado">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        @if (Session::has('success'))
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "{{ url('bienContable') }}";
            }
        });
        @endif
