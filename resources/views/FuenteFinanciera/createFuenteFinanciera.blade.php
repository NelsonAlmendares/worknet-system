          ...
          <div class="p-4">
            ...
            <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active" aria-current="page">Fuente Financiera</li>
                </ol>
              </nav>

            <div class="mt-5">
                <div class="card p-3">
                    <h3 class="text-center mb-3">Lista de Fuentes Financieras</h3>
                    @if($fuentes->isEmpty())
                        <p>Aún no hay fuentes financieras registrados.</p>
                    @else
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @foreach ($fuentes as $fuente)
                            <tr class="text-center">
                                <td>{{ $fuente->id_f_financiera }}</td>
                                <td>{{ $fuente->ff_nombre }}</td>
                                <td>{{ $fuente->ff_desc }}</td>
                                <td>{{ $fuente->estado_descripcion }}</td>
                                <td>

                                    <a href="{{ route('FuenteFinanciera.edit', $fuente->id_f_financiera) }}" class="btn btn-warning btn-sm">Editar</a>

                                    <form id="deleteForm-{{ $fuente->id_f_financiera }}" action="{{ route('FuenteFinanciera.destroy', $fuente->id_f_financiera) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm fw-bold" onclick="confirmDelete({{ $fuente->id_f_financiera }})">
            ...
            <div class="add-center">
                <button type="button" class="btn btn-secondary btn-custom-size fw-bold" data-bs-toggle="modal" data-bs-target="#exampleModal">
                    Agregar Fuente Financiera <i class='bx bx-user-pin custom-icon-size' ></i>
                </button>
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h1 class="modal-title fs-5" id="exampleModalLabel">Fuente Financiera</h1>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <div class="card-body">

                                <form action="{{ route('FuenteFinanciera.store') }}" method="POST">
                                    @csrf
                                    <!-- Nombre y Apellido -->
                                    <div class="mb-3">
                                        <label for="ff_nombre" class="form-label">Nombre</label>
                                        <input type="text" class="form-control" name="ff_nombre" id="ff_nombre" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="ff_desc" class="form-label">Descripción</label>
                                        <input type="text" class="form-control" name="ff_desc" id="ff_desc" required>
                                    </div>
@php
    $estados = [
            'A' => 'Activo',
...
@endphp
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="ff_e" name="ff_e" required>
                                            ...
                                            @foreach ($estados as $key => $label)
                                                <option value="{{ $key }}" {{ old('ff_e') == $key ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <button type="submit" class="btn btn-success fw-bold">
                                        Enviar Fuente Financiera <i class='bx bx-user-plus custom-icon-size' ></i>
                                    </button>
                                </form>
                            </div>
                        </div>
