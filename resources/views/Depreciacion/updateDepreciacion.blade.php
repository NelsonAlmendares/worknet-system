...
<div class="d-flex">
    ...
    <div class="main">
        <nav class="navbar navbar-expand-lg bg-body-tertiary">
          ...
          <div class="p-4">
            <h2 class="greet">Manejo de Depreciación</h2>

            <div class="mt-5">
                <div class="card container p-3">
                    <h3 class="text-center mb-3">Actualizar Depreciación</h3>

                    <form action="{{ route('Depreciacion.update', $depreciacion->afd_id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="afd_activo" name="afd_activo" required>
                                ...
                                @foreach ($activos as $activo)
                                    <option value="{{ $activo->id_activo }}" {{ old('afd_activo', $depreciacion->afd_activo) == $activo->id_activo ? 'selected' : '' }}>
                                        {{ $activo->a_nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="afd_vida_util" name="afd_vida_util" required>
                                ...
                                @foreach ($vidas as $vida)
                                    <option value="{{ $vida->id_vida_util_afd }}" {{ old('afd_vida_util', $depreciacion->afd_vida_util) == $vida->id_vida_util_afd ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" name="afd_fecha_generacion" id="afd_fecha_generacion" value="{{ $depreciacion->fecha_formateada_alt }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="afd_fecha_corte" class="form-label">Fecha de corte</label>
                            <input type="date" class="form-control" name="afd_fecha_corte" id="afd_fecha_corte" value="{{ $depreciacion->fecha_formateada_alt2 }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="afd_codigo_informe" class="form-label">Código de informe</label>
                            <input type="text" class="form-control" name="afd_codigo_informe" id="afd_codigo_informe" value="{{ $depreciacion->afd_codigo_informe }}" required>
                        </div>
                        @php
                            $estados = [
                            'A' => 'Activo',
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="afd_e" name="afd_e" required>
                                ...
                                @foreach ($estados as $key => $label)
                                    <option value="{{ $key }}" {{ old('afd_e', $depreciacion->afd_e) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="update-option">
                            <button type="submit" class="btn btn-primary espaciado">Actualizar Depreciación</button>
                            <a href="{{ route('Depreciacion.index') }}" class="btn btn-secondary espaciado">Cancelar</a>
                        </div>
                    </form>
                </div>
            </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        @if (Session::has('success'))
            ...
            if (result.isConfirmed) {
                ...
                window.location.href = "{{ url('depreciacion') }}";
            }
        });
        @endif
